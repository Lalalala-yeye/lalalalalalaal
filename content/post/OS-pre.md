...
}//这里写了命令的用法，usage：后面就是输出样例的指令，<op>表示操作名，<num>表示生成的个数，这就是为什么上面那么写
...
/* '<x>' 是需要替换的内容. */
/* use '.align <x>' to align the following words to 1-byte（希望是1字节一个对齐，所以就是.align 0） boundary (disabling word-alignment) */
